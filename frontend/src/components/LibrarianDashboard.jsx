...
const LibrarianDashboard = ({ token, onNavigateSettings, user }) => {
  ...
  const fetchDepartments = async () => {
    ...
    try {
      const res = await fetch(import.meta.env.VITE_API_URL + "/api/staff-departments", {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const fetchDocuments = async () => {
    try {
      const res = await fetch(import.meta.env.VITE_API_URL + "/api/documents/search", {
        headers: { Authorization: `Bearer ${token}` }
      });
      if (res.ok) {
  ...
  const handleArchive = async (id) => {
    if (!window.confirm("Are you sure you want to permanently archive this document?")) return;
  ...
  const handleApproval = async (id, status) => {
    ...
    if (status === "rejected") {
      rejection_reason = window.prompt("Please provide a reason for rejecting this document:");
      if (rejection_reason === null) return;
    }

    try {
      const res = await fetch(import.meta.env.VITE_API_URL + `/api/documents/${id}/approval`, {
        method: "PUT",
        headers: {
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const res = await fetch(import.meta.env.VITE_API_URL + `/api/documents/${editDocId}`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`
      ...
      if (res.ok) {
        ...
        const errorData = await res.json().catch(() => ({}));
        setEditMessage(`Error: ${errorData.error || "Failed to update document"}`);
      }
    } catch (err) {
  ...
  const filteredDocs = documents.filter(doc => {
    const matchesSearch = doc.title.toLowerCase().includes(searchQuery.toLowerCase()) || 
                          ...
                          (doc.serial_number && doc.serial_number.toLowerCase().includes(searchQuery.toLowerCase()));
      ...
      <main className={styles.mainContent}>
        ...
        <div className={styles.toolbar}>
          ...
          <button className={styles.uploadBtn} onClick={() => setIsUploadModalOpen(true)}>
            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24" width="20" height="20"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12" /></svg>
            Upload New Document
          </button>
        </div>
                    ...
                    <tr key={doc.id}>
                      ...
                      <td>
                        <div style={{ display: "flex", gap: "8px" }}>
                          <button className={styles.secondaryActionBtn} onClick={() => handlePreview(doc)}>Preview</button>
                          <button className={styles.primaryActionBtn} onClick={() => handleApproval(doc.id, "approved")}>Approve</button>
                          <button className={styles.dangerActionBtn} onClick={() => handleApproval(doc.id, "rejected")}>Reject</button>
                        </div>
                      </td>
                    </tr>
                  ))}
                ...
                {otherDocs.length > 0 ? (
                  otherDocs.map((doc) => (
                    <tr key={doc.id}>
                      ...
                      <td>
                        {(user?.role === 'System Administrators' || user?.role === 'Librarians' || doc.status === 'APPROVED') && (
                          <div style={{ display: "flex", gap: "8px" }}>
                            <button className={styles.secondaryActionBtn} onClick={() => handlePreview(doc)}>View</button>
                            <button className={styles.secondaryActionBtn} onClick={() => handleEditClick(doc)}>Edit</button>
                            <button className={styles.dangerActionBtn} onClick={() => handleArchive(doc.id)}>Archive</button>
                          </div>
                        )}
                      </td>
                    </tr>
                  ...
                  <tr>
                    <td colSpan="6" style={{ textAlign: "center", color: "#6b7280" }}>No documents found matching your criteria.</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            ...
            <form onSubmit={handleSave}>
              {message && <p style={{ color: message.startsWith("Error") ? "#ef4444" : "#10b981", marginBottom: "15px", fontWeight: "bold" }}>{message}</p>}
              ...
              <div className={styles.formGroup} style={{ display: 'flex', gap: '20px', alignItems: 'center', marginBottom: '15px' }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <input type="radio" checked={!isLink} onChange={() => setIsLink(false)} />
                  Upload Document
                </label>
                <label style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                  <input type="radio" checked={isLink} onChange={() => setIsLink(true)} />
                  Add External Link
                </label>
              </div>
              ...
              {!isLink ? (
                <div className={styles.formGroup}>
                  ...
                  <input id="librarianFileInput" type="file" accept=".pdf,.doc,.docx,.xls,.xlsx,.csv,.ppt,.pptx,.jpg,.jpeg,.png,.svg,.txt,.md,.zip,.gz" onChange={(e) => setFile(e.target.files[0])} required />
                </div>
              ) : (
                <>
                  <div className={styles.formGroup}>
                    <label>External URL</label>
                    <input type="url" placeholder="https://example.com/reference" value={linkUrl} onChange={(e) => setLinkUrl(e.target.value)} required />
                  </div>
                  <div className={styles.formGroup}>
              ...
              <div className={styles.modalActions}>
                <button type="button" className={styles.secondaryActionBtn} onClick={() => setIsUploadModalOpen(false)}>Cancel</button>
                <button type="submit" className={styles.uploadBtn}>Upload to Library</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContent}>
            ...
            <form onSubmit={handleUpdate}>
              {editMessage && <p style={{ color: "#ef4444", marginBottom: "15px", fontWeight: "bold" }}>{editMessage}</p>}
              ...
              <div className={styles.formGroup}>
                <label>Serial Number</label>
                <input type="text" value={editSerial} onChange={(e) => setEditSerial(e.target.value)} required />
              </div>

              <div className={styles.modalActions}>
                <button type="button" className={styles.secondaryActionBtn} onClick={() => setIsEditModalOpen(false)}>Cancel</button>
                <button type="submit" className={styles.uploadBtn}>Save Changes</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className={styles.modalOverlay}>
          <div className={styles.modalContentPreview}>
            <div className={styles.sectionHeader}>
              <h2 style={{ margin: 0, fontSize: "20px", color: "#1f2937" }}>Viewing: {previewDoc.title}</h2>
              <button className={styles.secondaryActionBtn} onClick={closePreview}>Close Viewer</button>
            </div>
            <iframe src={previewUrl} style={{ width: "100%", flex: 1, border: "1px solid #e5e7eb", borderRadius: "8px" }} title="Document Preview" />
            
            {/* Show Approve/Reject ONLY if the doc is pending */}
            {previewDoc.status === "pending" && (
              <div style={{ display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "20px", borderTop: "1px solid #e5e7eb", paddingTop: "20px" }}>
                <button className={styles.primaryActionBtn} onClick={() => handleApproval(previewDoc.id, "approved")}>Approve Document</button>
                <button className={styles.dangerActionBtn} onClick={() => handleApproval(previewDoc.id, "rejected")}>Reject Document</button>
              </div>
            )}
          </div>
        </div>
