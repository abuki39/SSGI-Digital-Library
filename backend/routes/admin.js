...
// PUT full user update (Name, Phone, Role, Department)
router.put("/users/:id", async (req, res) => {
  ...
  try {
    ...
    // If a department was selected, fetch its actual name based on the role
    if (department_id) {
    ...
    await db.execute(
      "UPDATE users SET first_name = ?, last_name = ?, phone_number = ?, role_id = ?, department = ?, department_id = ? WHERE id = ?",
      [
        first_name,
        last_name,
        phone_number,
...
// PUT role update (Maintained for backwards compatibility if needed)
