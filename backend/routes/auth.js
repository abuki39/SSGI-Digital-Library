...
router.post("/login", async (req, res) => {
  ...
  try {
    // ============================================================
    // GET USER
    // Includes profile information, lockout tracking,
    // email alerts, and theme preference.
    // ============================================================
    ...
    if (!user.department || !user.department_id) {
      try {
        ...
        if (mockMatch) {
          if (!user.department) {
            user.department = mockMatch.department;
          }

          if (!user.department_id) {
            user.department_id = mockMatch.department_id;
          }
        }

        // If department string is still missing
        // but we have an ID, resolve it.
        if (!user.department && user.department_id) {
          let deptStr = "General";
          ...
          if (staffDept) {
            deptStr = staffDept.name;
          } else if (regDept) {
            deptStr = regDept.name;
          }
    ...
    // ============================================================
    //
    // Existing users may have NULL values because the columns
    // already existed before the preference feature was added.
    //
    // Default behavior:
    // email_alerts     -> ON
    // theme_preference -> light
    //
    // These defaults are also saved back to the database.
    // ============================================================

    if (user.email_alerts === null || user.email_alerts === undefined) {
      user.email_alerts = 1;

      await db.execute("UPDATE users SET email_alerts = ? WHERE id = ?", [
        1,
        user.id,
      ]);
    } else {
      user.email_alerts = Number(user.email_alerts);
    }

    if (
      !user.theme_preference ||
      !["light", "dark", "system"].includes(user.theme_preference)
    ) {
      user.theme_preference = "light";

      await db.execute("UPDATE users SET theme_preference = ? WHERE id = ?", [
        "light",
        user.id,
      ]);
    }
    ...
    res.json({
      ...
      user: {
        ...
        // User Preferences
        email_alerts: Boolean(Number(user.email_alerts)),
...
router.put("/change-password", authenticateToken, async (req, res) => {
  ...
  try {
    ...
    // Preserve existing mock fallback behavior
    res.json({
      message: "Password updated successfully (mock fallback)",
    });
  }
});
